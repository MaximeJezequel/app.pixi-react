...
const OptionBlock2 = ({ tags, setTags }) => {
  ...
  const addTag = (e) => {
    if (tags.includes(newTag)) {
      ...
      alert('Snackbar: Maximum 6 tags authorized, please delete a tag before creating a new one !');
    } else {
      setTags([...tags, newTag]);
      e.target.value = '';
      setNewTag('');
        ...
        <Dialog open={open} onClose={() => setOpen(false)}>
          ...
          >
            <TextField
              ...
              onKeyDown={(e) => (updateTag.length > 0 ? e.key === 'Enter' && modifyTag() : null)}
            />
          </DialogContent>
          <DialogActions
            sx={{
